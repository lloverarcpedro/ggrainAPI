...
const addLoadToContract = async (req: Request): Promise<string> => {
    ...
    const contractSubmit = await submitTxnContract(contract, contractId, loadId, commodityId, weight, sellerId, moisture)
    ...
    return await submitTxnTransaction(contract, contractId, loadId, commodityId, weight, sellerId, moisture)
 ...
 */
async function submitTxnContract(contract: Contract, contractId: string, loadId: string, commodityId: string, weight: string, sellerId: string, moisture: string) {
    try {
        // Submit the transaction
        const response = await contract.submitTransaction('addLoad',contractId, loadId, commodityId, weight, sellerId, moisture)
 ...
 * @param {object} contract 
 */
async function submitTxnTransaction(contract: Contract, contractId: string, loadId: string, commodityId: string, weight: string, sellerId: string, moisture: string) {
    ...
    // Submit the transaction
    try {
        const response = await txn.submit(contractId, loadId, commodityId, weight, sellerId, moisture)
...
export { getLoadById, addLoadToContract }